...
export function productCard(p) {
  return `
    <article class="card reveal" data-id="${p.id}">
      ...
      <button class="card__quick" data-quick="${p.id}" aria-label="Quick add to cart">Quick Add</button>
      <a href="product.html?id=${p.id}" style="display:contents">
        ...
        <div class="card__row">
          ...
          <div class="card__rating">${p.rating.toFixed(1)} <span style="color:var(--muted)">(${p.reviews})</span></div>
        </div>
      </a>
    </article>`;
}
